...
RUN_NAME = "ReZero+ResNet+NRMSE"  # | input("enter name for run")
...
MODEL_CONFIG = {
    ...
    "Filter": FILTER,
    "System": SYSTEM,
    "Encoder": ResNet.DEFAULT_HP | {"rezero": True},
    "Decoder": ResNet.DEFAULT_HP | {"rezero": True},
    "Embedding": EMBEDDING,
}
...
ax[0, 0].set_title(
    r"Rel. Magnitude change $\hat{x}_0  \rightarrow \hat{x}_{t+1}  $"
)
...
ax[0, 2].set_title(
    r"Rel. Magnitude change $\hat{z}_0  \rightarrow \hat{z}_{t+1}  $"
)
...
ax[1, 0].set_title(
    r"Rel. Magnitude change $\hat{x}_t  \rightarrow \hat{x}_{t+1}  $"
)
...
ax[1, 2].set_title(
    r"Rel. Magnitude change $\hat{z}_t  \rightarrow \hat{z}_{t+1}  $"
)
...
ax[2, 1].semilogy(encoder_mag)
ax[2, 1].set_title(r"Rel. magnitude change $\hat{x}_t' \rightarrow \hat{z}_t'$")
ax[2, 2].semilogy(system_mag)
...
fig.savefig(f"{RUN_NAME}_encoder_stats.pdf")
...
fig, axes = plt.subplots(nrows = 2, ncols=2, constrained_layout=True, figsize=(8,5), sharex=True, sharey=True)
