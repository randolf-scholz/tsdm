...
TD_NUMPY_DUR: NDArray[np.timedelta64] = np.array(
    [PY_TIMEDELTA], dtype="timedelta64[ms]"
)
...
TD_PANDAS_NP_DUR: pd.Series = pd.Series([PY_TIMEDELTA], dtype="timedelta64[ms]")
TD_PANDAS_NP_FLOAT: pd.Series = pd.Series([PY_FLOAT], dtype="float64")
TD_PANDAS_NP_INT: pd.Series = pd.Series([PY_INT], dtype="int64")
TD_PANDAS_PA_DUR: pd.Series = pd.Series([PY_TIMEDELTA], dtype="duration[ms][pyarrow]")
...
DURATION_ARRAYS: dict[KEY, SpanLikeArray] = {
    "numpy[np_float]"  : TD_NUMPY_FLOAT,
    "numpy[np_int]"    : TD_NUMPY_INT,
    "numpy[np_time]"   : TD_NUMPY_DUR,
    ...
    "polars[pa_float]" : TD_POLARS_FLOAT,
    "polars[pa_int]"   : TD_POLARS_INT,
    "polars[pa_time]"  : TD_POLARS_DUR,
}  # fmt: skip
r"""Dictionary of timedelta arrays."""

DURATION_SCALARS: dict[KEY, SpanLikeScalar] = {
    "numpy[np_float]"  : NP_FLOAT,
    "numpy[np_int]"    : NP_INT,
    "numpy[np_time]"   : NP_TIMEDELTA,
    "pandas[np_time]"  : PD_TIMEDELTA,
    "pandas[np_float]" : PY_FLOAT,
    "pandas[np_int]"   : PY_INT,
    "pandas[pa_time]"  : PD_TIMEDELTA,
    "pandas[pa_float]" : PY_FLOAT,
    "pandas[pa_int]"   : PY_INT,
    "polars[pa_time]"  : PY_TIMEDELTA,
    "polars[pa_float]" : PY_FLOAT,
    "polars[pa_int]"   : PY_INT,
}  # fmt: skip
...
TIMESTAMP_ARRAYS: dict[KEY, TimeLikeArray] = {
    "numpy[np_float]"  : TS_NUMPY_FLOAT,
    "numpy[np_int]"    : TS_NUMPY_INT,
    "numpy[np_time]"   : TS_NUMPY_DATE,
    ...
    "polars[pa_time]"  : TS_POLARS_DATE,
    "polars[pa_float]" : TS_POLARS_FLOAT,
    "polars[pa_int]"   : TS_POLARS_INT,
}  # fmt: skip
r"""Dictionary of timestamp arrays."""

DATETIME_ARRAYS: dict[KEY, DatetimeArray] = {
    "numpy[np_time]"   : TS_NUMPY_DATE,  # type: ignore[dict-item]  # pyright: ignore[reportAssignmentType]
    "pandas[np_time]"  : TS_PANDAS_NP_DATE,
    "pandas[pa_time]"  : TS_PANDAS_PA_DATE,
    "polars[pa_time]"  : TS_POLARS_DATE,
}  # fmt: skip
...
TIMESTAMP_SCALARS: dict[KEY, TimeLikeScalar] = {
    "numpy[np_float]"  : NP_FLOAT,
    "numpy[np_int]"    : NP_INT,
    "numpy[np_time]"   : NP_DATETIME,  # type: ignore[dict-item]
...
class TestDurationArrayProtocol:
    ...
    # float arrays
    _numpy_float: SpanLikeArray[float] = TD_NUMPY_FLOAT
    _pandas_np_float: SpanLikeArray[float] = TD_PANDAS_NP_FLOAT
    _pandas_pa_float: SpanLikeArray[float] = TD_PANDAS_PA_FLOAT
    _polars_float: SpanLikeArray[float] = TD_POLARS_FLOAT
    # int arrays
    _numpy_int: SpanLikeArray[int] = TD_NUMPY_INT
    _pandas_np_int: SpanLikeArray[int] = TD_PANDAS_NP_INT
    _pandas_pa_int: SpanLikeArray[int] = TD_PANDAS_PA_INT
    _polars_int: SpanLikeArray[int] = TD_POLARS_INT
    # timedelta arrays
    # _numpy_time: DurationArray[py_timedelta] = TD_NUMPY_DUR
    _pandas_np_time: SpanLikeArray[py_timedelta] = TD_PANDAS_NP_DUR
    _pandas_pa_time: SpanLikeArray[py_timedelta] = TD_PANDAS_PA_DUR
    _polars_time: SpanLikeArray[py_timedelta] = TD_POLARS_DUR
    ...
    DURATION_TIMEDELTA_ARRAYS: dict[KEY, SpanLikeArray[py_timedelta]] = {
        # "numpy[np_time]"     : TD_NUMPY_DUR,
        "pandas[np_time]": TD_PANDAS_NP_DUR,
        "pandas[pa_time]": TD_PANDAS_PA_DUR,
        "polars[pa_time]": TD_POLARS_DUR,
    ...
    def test_duration_arrays(self, case: str) -> None:
        ...
        assert type(td_array + td_scalar) is cls
        assert type(td_array - td_scalar) is cls
        assert type(ts_scalar + td_array) is cls  # type: ignore[unreachable]
        assert type(ts_scalar - td_array) is cls  # type: ignore[unreachable]
    ...
    def test_timedelta_arrays(self, case: str) -> None:
        ...
        if not TYPE_CHECKING:
            with pytest_xfail(condition=case == "numpy[np_time]", defer_xfail=True):
                assert type(td_array + PY_DATETIME) is cls
            with pytest_xfail(condition=case == "numpy[np_time]", defer_xfail=True):
                assert type(PY_DATETIME + td_array) is cls
            with pytest_xfail(condition=case == "numpy[np_time]", defer_xfail=True):
                assert type(td_array + PY_TIMEDELTA) is cls
            with pytest_xfail(condition=case == "numpy[np_time]", defer_xfail=True):
...
class TestTimestampArrayProtocol:
    ...
    # float arrays
    _numpy_float: TimeLikeArray[float] = TS_NUMPY_FLOAT
    _pandas_np_float: TimeLikeArray[float] = TS_PANDAS_NP_FLOAT
    _pandas_pa_float: TimeLikeArray[float] = TS_PANDAS_PA_FLOAT
    _polars_float: TimeLikeArray[float] = TS_POLARS_FLOAT

    # int arrays
    _numpy_int: TimeLikeArray[int] = TS_NUMPY_INT
    _pandas_np_int: TimeLikeArray[int] = TS_PANDAS_NP_INT
    _pandas_pa_int: TimeLikeArray[int] = TS_PANDAS_PA_INT
    _polars_int: TimeLikeArray[int] = TS_POLARS_INT

    # datetime arrays
    # _numpy_time: TimestampArray[py_timedelta] = TS_NUMPY_DATE
    _pandas_np_time: TimeLikeArray[py_timedelta] = TS_PANDAS_NP_DATE
    _pandas_pa_time: TimeLikeArray[py_timedelta] = TS_PANDAS_PA_DATE
    _polars_time: TimeLikeArray[py_timedelta] = TS_POLARS_DATE
    ...
    r"""Dictionary of int arrays."""
    TIMESTAMP_PYDATETIME_ARRAYS: dict[KEY, TimeLikeArray[py_datetime]] = {
        # "numpy[np_time]"     : TS_NUMPY_DATE,
        "pandas[np_time]": TS_PANDAS_NP_DATE,
        "pandas[pa_time]": TS_PANDAS_PA_DATE,
        "polars[pa_time]": TS_POLARS_DATE,
