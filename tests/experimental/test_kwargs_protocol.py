...
class _SupportsKwargsMeta(ProtocolMeta):
    ...
    def __instancecheck__(cls, instance: object, /) -> TypeIs[SupportsKwargs]:  # ruff: ignore[N805]
        return isinstance(instance, SupportsKeysAndGetItem) and all(
            isinstance(key, str)
            for key in instance.keys()  # ruff: ignore[SIM118]
        )

    def __subclasscheck__(cls, subclass: type, /) -> TypeIs[type[SupportsKwargs]]:  # ruff: ignore[N805]
