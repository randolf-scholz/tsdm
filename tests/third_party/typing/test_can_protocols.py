...
def test_can_self_overloaded_implementation() -> None:
    ...
    class A:  # (X -> A), (A -> A)
        @overload
        def op(self, other: X, /) -> A: ...
        @overload
        def op(self, other: A, /) -> A: ...

    class A_Self:  # (X -> Self), (Self -> Self)
        @overload
        def op(self, other: X, /) -> Self: ...
        @overload
        def op(self, other: Self, /) -> Self: ...

    class B:  # (B -> B), (X -> B)
        @overload
        def op(self, other: B, /) -> B: ...
        @overload
        def op(self, other: X, /) -> B: ...
...
def test_can_self_overloaded_protocol() -> None:
    ...
    class CanSelfOP_Union(Protocol):  # (X | Self) -> Self
        def op(self, other: X | Self, /) -> Self: ...

    class A:  # (A | X) -> A
        def op(self, other: A | X, /) -> A: ...

    class A_Self:  # (Self | X) -> Self
        def op(self, other: Self | X, /) -> Self: ...
    ...
    class C:  # (X -> C), (C -> C)
        @overload
        def op(self, other: X, /) -> C: ...
        @overload
        def op(self, other: C, /) -> C: ...
