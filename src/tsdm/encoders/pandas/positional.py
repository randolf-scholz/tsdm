...
class PeriodicEncoder(FittableEncoder[pd.Series, pd.DataFrame]):
    ...
    def encode(self, x: pd.Series, /) -> pd.DataFrame:
        ...
        return pd.DataFrame(np.stack([np.cos(z), np.sin(z)]).T, columns=columns)

    def decode(self, y: pd.DataFrame, /) -> pd.Series:
        r"""Decode the data."""
        columns = [f"cos_{self.original_name}", f"sin_{self.original_name}"]
        z = np.arctan2(y[columns[1]], y[columns[0]])
        z = (z / self.freq) % self.period
        return pd.Series(z, dtype=self.original_dtype, name=self.original_name)
